...
function preload()
{
	...
	jumpso=loadSound("sounds/jump.mp3");
	hit=loadSound("sounds/hit.mp3");
	go=loadSound("sounds/loose.mp3");
	wow=loadSound("sounds/win.mp3");
}
...
function setup() {
	...
	//enemy
	mainen=createSprite(displayWidth-80,535,20,20);
...
function draw() {
  ...
  if(gameState === 0){
	...
	stroke("red");
	strokeWeight(5);
	text("Super Castle",610,100);
	text("Coin Treasure",600,200);
	...
	stroke("black");
	strokeWeight(4);
	text("Run by pressing the buttons",50,120);
	text("And reach the castle",50,150);
	text("And collect the coins",50,180);
	text("Dont get hit by the bullets, save him by jumping",50,210);
	text("Your hearts will be decreased",50,240);
	text("Collect 50 coins to win",50,270);
	text("Game will get harder as you collect more coins",50,300);
	text("You will get 2 chances to collect 50 coins",50,330);
	text("If you loose, then You have to restart the game again",50,360);
	text("Click on start to play",50,420);
  ...
  if(gameState === 1){
	  ...
	  text("Way to the castle",1100,100);
	   //spawn the clouds
	   spawnClouds();
	  //game time
  ...
  if(gameState===2){
	  ...
	  text("Inside the castle",1100,100);
  ...
  if(gameState===4){
	...
	textFont("Cooper Black")
	fill("white");
	text("You have successfully collected",100,displayHeight/2-300);
	text("all the coins in the castle 😊",100,displayHeight/2-200);
	fill("yellow");
	text("Coins Collected : "+ coins,300, displayHeight/2);
